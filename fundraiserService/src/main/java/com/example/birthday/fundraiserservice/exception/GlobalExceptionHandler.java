package com.example.birthday.fundraiserservice.exception;
...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiError> conflict(HttpServletRequest request) {
        return response(HttpStatus.CONFLICT, "Participant already joined", request);
    }
    @ExceptionHandler(FeignException.NotFound.class)
    ResponseEntity<ApiError> externalNotFound(HttpServletRequest request) {
        return response(HttpStatus.UNPROCESSABLE_ENTITY, "Related resource does not exist", request);
    }
    @ExceptionHandler(FeignException.class)
    ResponseEntity<ApiError> external(HttpServletRequest request) {
        return response(HttpStatus.SERVICE_UNAVAILABLE, "Dependent service is unavailable", request);
    }
