package com.example.birthday.subscriptionservice.exception;
...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiError> duplicate(HttpServletRequest request) {
        return response(HttpStatus.CONFLICT, "Active subscription already exists", request);
    }
    @ExceptionHandler(FeignException.NotFound.class)
    ResponseEntity<ApiError> targetNotFound(HttpServletRequest request) {
        return response(HttpStatus.UNPROCESSABLE_ENTITY, "Subscription target does not exist", request);
    }
    @ExceptionHandler(FeignException.class)
    ResponseEntity<ApiError> external(HttpServletRequest request) {
        return response(HttpStatus.SERVICE_UNAVAILABLE, "Target service is unavailable", request);
    }
